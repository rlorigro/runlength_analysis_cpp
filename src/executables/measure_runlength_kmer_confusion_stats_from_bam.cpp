...
void chunk_sequences_into_regions(vector<Region>& regions, unordered_map<string,FastaIndex>& index, uint64_t chunk_size){
    ///
    /// Take all the sequences in some iterable object and chunk their lengths
    ///

    // For every sequence
    for (auto& [name, item]: index){
...
void measure_kmer_stats_from_fasta(path bam_path,
    ...
    // only one alignment worth of RAM is consumed per chunk. Larger chunks reduce overhead
...
int main(int argc, char* argv[]){
    ...
    options.add_options()
        ...
        ("bam",
        value<path>(&bam_path),
        "File path of BAM file containing alignment of QUERY to REF")
    ...
    measure_kmer_stats_from_fasta(bam_path,
            ref_fasta_path,
            reads_fasta_path,
            output_dir,
            max_threads,
